# -*- coding: utf-8 -*-
"""mdoeling_for_0116_to_0122.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1OV2PGmuhDR_J3l0Blk1MIX_Sy24kmlaP
...
def golditem_preprocessing(golditem,y):
    ...
    golditem = golditem.drop(['logtype','userclienttype'], axis=1)
    ## Y結果表, Golditem 資料表串接
...
login_log = pd.read_csv("login_log_for_0116_to_0122_reserve_brokentime.csv",index_col = 'Unnamed: 0')
y = pd.read_csv("click_charge_0116_0122_statics_202206301102.csv")
...
charge = pd.read_csv("charge_for_0116_to_0122_reserve_brokentime.csv")
...
BrokenPlayerDepositClick = pd.read_csv('depositclicklog_0116_0122_before_34hrs_202206251817.csv')
# 讀 gameinfo 相關數據(X)
BrokenPlayerGameInfo_data = pd.read_csv('tmd_gameinfo_playresult_broken_duration_0116_0122_202206251316-001.csv')     # 讀破產玩家遊戲行為數據(破產前34hr)
Themeid_to_cate_data = pd.read_csv('Themeid_to_cate.csv')[['Themeid','en_cate']]     # 讀遊戲對應表數據
...
X = df_merge.drop([
                    'accountid',
                    'broken_time_end',
                    'broken_time_start_charge',
                    'broken_time_start_login',
                    'broken_time_clickbrokenpackage_end_charge',
                    'isclick_charge',
                    'sum_click_charge',
                    'sum_click_charge_money_charge',
                    'broken_time_start', 
                    'broken_time_clickbrokenpackage_end', 
                    'isclick',
                    'sum_click',
                    'sum_click_charge_money', 
                    'broken_time_clickbrokenpackage_end_login', 
                    'isclick_login',
                    'sum_click_login', 
                    'sum_click_charge_money_login', ],axis=1).values
...
input_columns = df_merge.drop([
                                ...
                                'isclick_login',
                                'sum_click_login',
                                'sum_click_charge_money_login', ],axis=1).columns.tolist()
...
giftlog = pd.read_csv('giftlog_for_0116_to_0122_reserve_brokentime.csv')
#golditem = pd.read_csv('golditem_for_0116_reserve_brokentime.csv')
...
golditem = pd.read_csv('golditem_for_0116_to_0122_reserve_brokentime-002.csv')#這個路徑還沒有改，因為上面的def要重寫
...
training_df = y.groupby(['accountid','broken_time_end']).size().reset_index().drop(0,axis=1)
for index in range(len(preconcated_df)):
    temp = preconcated_df[index].reset_index()
    temp['broken_time_end'] = pd.to_datetime(temp['broken_time_end'])
    exec(f"training_df = pd.merge(training_df,temp,left_on = ['accountid','broken_time_end'],right_on = ['accountid','broken_time_end'])")
training_df

#training_df = pd.concat(preconcated_df,axis=1)
#training_df.reset_index(inplace =True)
BrokenClickTarget['broken_time_end'] = pd.to_datetime(BrokenClickTarget['broken_time_end'])
...
xgb =XGBClassifier(subsample= 0.6, n_estimators= 600, min_child_weight= 5, max_depth= 4, gamma= 1.5, colsample_bytree= 1.0).fit(X_train,Y_train)
...
estimators = [
    ...
    ('XGBClassifier', XGBClassifier(subsample= 0.6, n_estimators= 600, min_child_weight= 5, max_depth= 4, gamma= 1.5, colsample_bytree= 1.0)),
